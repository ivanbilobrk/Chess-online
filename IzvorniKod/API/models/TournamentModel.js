    ...
    static getAllScheduledTournaments = async(id) => {
        ...
        if(tournaments?.length != 0){
            for(let i = 0; i < tournaments.length; i++){
                let newTournament = new Tournament(tournaments[i].trainer_id, tournaments[i].tournamentstarttimedate, tournaments[i].tournamentdurationmin);
                newTournament.id = tournaments[i].id;
                newTournament.participantsNo = tournaments[i].participantsNo;
                newTournament.showing = tournaments[i].showing;
                result[i] = newTournament;
            }
            console.log(result);
            return result;
        }
...
const dbGetAllScheduledTournaments = async (id)=>{
    const sql = "select * from scheduledTournament join users on member_id = id where member_id =" + id;
...
const dbCancelTournament = async (tournament) => {
    const sql = "delete from scheduledtournament where tournament_id = '" + tournament.id + "'";
